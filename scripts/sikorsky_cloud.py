...
from simulib.simulib.utils import c0, DTR, _complex_float
...
if __name__ == "__main__":
    ...
    e = np.zeros(n_total_gps) + 150. + np.linspace(0, 100, n_total_gps)
    ...
    # Position of gimbal
    gim_pan = sawtooth(gps_times / cfig.radar_params.scan_rate, width=.5) * cfig.radar_params.scan_limit * DTR / 2
    gim_el = np.zeros_like(gps_times)
    gimbal = np.array([gim_pan, gim_el]).T

    pulse_times = np.arange(int(gps_times[-1] * prf)) / prf

    rx_offsets = np.array([[-wavelength / 4, wavelength / 4, 0],
                           [-wavelength / 4, -wavelength / 4, 0],
                           [wavelength / 4, wavelength / 4, 0],
                           [wavelength / 4, -wavelength / 4, 0]])

    tx_offsets = np.array([[-wavelength / 2, 0, 0],
                           [wavelength / 2, 0, 0]])
    weights = np.array([[1., 1., 1., 1.]])
    goffsets = np.array([0, np.pi / 2, 0.])
    rx_array = rx_offsets.dot(getRotationOffsetMatrix(*goffsets))
    tx_array = tx_offsets.dot(getRotationOffsetMatrix(*goffsets))
    vx_array = np.concatenate([rx_array + tx for tx in tx_array])

    rp = RadarPlatform(e, n, u, r, p, y, gps_times, tx_offsets, rx_offsets, gimbal=gimbal,
                  gimbal_offset=np.array([0., 1., 0.]), gimbal_rotations=goffsets, dep_angle=0.,
                  az_bw=bw_az / DTR, el_bw=bw_el / DTR, fs=fs, fc=fc, prf=prf, bwidth=bandwidth)

    nsam, nr, ranges, ranges_sampled, near_range_s, granges, fft_len, up_fft_len = rp.getRadarParams(u.mean(), .5, a_ranges=init_ranges)
    ...
    for frame in tqdm(list(zip(*(iter(range(0, len(pulse_times), npulses)),)))):
        ...
        if len(ptimes) < npulses:
            break
        txposes = np.expand_dims(rp.txpos(ptimes)[..., :3].swapaxes(0, 1), axis=1)
        rxposes = np.expand_dims(rp.rxpos(ptimes)[..., :3].swapaxes(0, 1), axis=1)
        block_data = trace_cpi(tracer, chirps,
                                npulses, txposes, rxposes, weights,
                                rp._tx.boresight(ptimes).T, materials, ptimes,
        ...
        if num_blobs > 0:
            for b in range(num_blobs):
                ...
                dirs = azelToVec(fine_az_mesh[best_pts] + rp.az_iner(ptimes).mean(),
                                 fine_el_mesh[best_pts] + rp.el_iner(ptimes).mean()).T
                poss_pts = rp.pos(ptimes).mean(axis=0) + dirs.mean(axis=0) * ranges[blob_idx].mean()
                pcloud0.append(poss_pts)
                apr = az_del_data / sum_data
                epr = el_del_data / sum_data
                target_angles = np.array([[rp.az_iner(ptimes).mean(),
                                           rp.el_iner(ptimes).mean()] for _ in blob_x])
                target_angles[:, 0] += np.arcsin(
                    (c0 / fc) / (np.pi * np.linalg.norm(rx_offsets[0] - rx_offsets[2])) * np.arctan(apr.imag[blob_x, blob_y]))
                target_angles[:, 1] += np.arcsin(
                    (c0 / fc) / (np.pi * np.linalg.norm(rx_offsets[0] - rx_offsets[2])) * np.arctan(epr.imag[blob_x, blob_y]))
                target_ranges = ranges[blob_y]
                final_angle = np.sum(target_angles * det_sum[blob_x, blob_y][:, None], axis=0) / sum(det_sum[blob_x, blob_y])
                pcloud1.append(rp.pos(ptimes).mean(axis=0) + target_ranges.mean() * azelToVec(*final_angle).T)

        # Get angle from boresight
        '''if len(poss_targ[0]) > 0:
            apr = az_del_data / sum_data
            epr = el_del_data / sum_data
            target_angles = np.array([[rp.az_iner[rp.prev(npulses)][-1],
                                       rp.el_iner[rp.prev(npulses)][-1]] for _ in poss_targ[0]])
            target_angles[:, 0] += np.arcsin((c0 / fc) / (np.pi * np.linalg.norm(rx_offsets[0] - rx_offsets[1])) * np.arctan(apr.imag[poss_targ]))
            target_angles[:, 1] += np.arcsin((c0 / fc) / (np.pi * np.linalg.norm(rx_offsets[2] - rx_offsets[3])) * np.arctan(epr.imag[poss_targ]))
            target_ranges = ranges[poss_targ[1]]
            target_est = rp.pos[rp.tell] + target_ranges[:, None] * azelToVec(target_angles[:, 0], target_angles[:, 1]).T'''


        '''plt.axis('tight')
        im = ax.imshow(db(rpd.T), origin='lower', extent=(dopp_freq[0], dopp_freq[-1], ranges[0], ranges[-1]))
        ...
        if abs(rp.az_iner(ptimes).mean() - np.arctan2(base_pos[0, 0], base_pos[0, 1])) < bw_az:
            sumax.scatter([0], [np.linalg.norm(rp.txpos(ptimes).mean(axis=0))], c='red')
