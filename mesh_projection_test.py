...
from simulation_functions import db, genChirp, upsamplePulse, findPowerOf2, factors
...
print('Loading mesh...')
mesh = readCombineMeshFile('/home/jeff/Documents/plot.obj', points=20000)  # Has just over 243000 points in the file
...
face_factors = factors(face_points.shape[0])
face_reshape = (face_factors[len(face_factors) // 2], int(face_points.shape[0] / face_factors[len(face_factors) // 2]))
gx = face_points[:, 0].reshape(face_reshape)
gy = face_points[:, 1].reshape(face_reshape)
gz = face_points[:, 2].reshape(face_reshape)
...
box_tree, sample_points = getBoxesSamplesFromMesh(mesh, num_boxes=4, sample_points=10000)
...
'''bpj_grid = backprojectPulseSet(pulses, rp.pan(data_t), rp.tilt(data_t), rp.rxpos(data_t), rp.txpos(data_t), gx, gy, gz,
